package com.bamenyouxi.room_card_mode.web.web_api.sqlserver;
...
import com.bamenyouxi.room_card_mode.service.sqlserver.account.AccountsInfoService;
...
final class AccountsInfoController extends AbstractCrudController<AccountsInfo, Integer> {
	...
	@GetMapping("/giveAgent/{gameId}")
	public WebResult giveAgent(@PathVariable Integer gameId){
		List<AccountsInfo> list=accountsInfoMapper.giveAgent(gameId);
		if(list!=null&&!list.isEmpty()){
			boolean s=list.get(0).getLoveLiness()==1;
			if(s){
				Assert.isTrue(true,TipMsgConstant.AGENT_LOOK_CARD);
				return  WebResult.of(list.get(0));
			}
			int n=accountsInfoMapper.updateGiveAgent(gameId);
			Assert.isTrue(n > 0, TipMsgConstant.OPERATION_FAILED);
			return WebResult.of(list.get(0));
		}
		Assert.isTrue(list.size()<0, TipMsgConstant.SYS_PLAY);
	...
	public WebResult queryGiveAgent(
			...
			@RequestParam(value = SysConstant.PageConstant.DEFAULT_SIZE_NAME,defaultValue = ""+SysConstant.PageConstant.DEFAULT_SIZE) Integer size,
			@RequestParam Map<String, Object> params){
		return WebResult.of(accountsInfoService.queryGiveAgent(page,size,params));
	}

	@PutMapping("/authorizeGiveAgent/{gameId}")
	public WebResult authorizeGiveAgent(@PathVariable int gameId){
		int n=accountsInfoMapper.authorizeGiveAgent(gameId);
		Assert.isTrue(n > 0, TipMsgConstant.OPERATION_FAILED);
		return WebResult.of();
	}
