 ...
 */
@Service
public class SysGoldClearExcelService {
	@Autowired
	...
	private ClearExcel.Builder buildTitle(String startDate, String endDate) {
		return
			new ClearExcel.Builder()
				...
				.child(new StringBuffer(startDate).append("至").append(endDate).append("代理金币消耗结算报表.xlsx").toString())
				.sheetName("代理结算报表")
	...
	private List<SysAgent> getClearList(String startDate, String endDate) {
		...
		list.forEach(sysAgent -> {
			...
			for (UserGold userGold : this.userGoldMapper.sumActual(new HashMap<String, Object>() {{
				put(FieldConstant.ModelFieldConstant.superAgentGameId.name(), sysAgent.getGameId());
				put(FieldConstant.CommonFieldConstant.t1.name(), goldInfo.getT1_commission());
				put(FieldConstant.CommonFieldConstant.t2.name(), goldInfo.getT2_commission());
				put(FieldConstant.CommonFieldConstant.t3.name(), goldInfo.getT3_commission());
				put(FieldConstant.CommonFieldConstant.startDate.name(), startDate);
				put(FieldConstant.CommonFieldConstant.endDate.name(), endDate);
			}}))
				clearPrice = clearPrice.add(userGold.getCommission());
